...
open class Node {
    ...
    public init() {
        ...
        let measureFunction: @convention(c) (UnsafeMutableRawPointer?, Float, Float) -> css_dim_t = { (context, width, height) in
        ...
        let getChildFunction: @convention(c) (UnsafeMutableRawPointer?, Int32) -> UnsafeMutablePointer<css_node>? = { (context, idx) in
            let node = Unmanaged<Node>.fromOpaque(context!).takeUnretainedValue()
    ...
    open func reset() {
        ...
        style.maxDimensions = (Float.greatestFiniteMagnitude, Float.greatestFiniteMagnitude)
    ...
    open func layout(_ maxWidth: Float = Undefined, maxHeight: Float = Undefined, parentDirection: Directive.Direction = .inherit) {
        resetLayout()
